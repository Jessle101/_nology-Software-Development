...
const firstArray = [true, true, false, false, true];

function removeFalseValues(booleanArr) {
  if (booleanArr === true && booleanArr !== false) {
    return booleanArr;
  }
}

const truesArray = firstArray.filter(removeFalseValues);
console.log(truesArray);

const removeFalseArrays = (firstArray) => {
  const truesArray = firstArray.filter(checkFalseValues);
  console.log(truesArray);
  return truesArray;
};
...
const decimalArray = [0.29, 0.6942, 0.38, 1, 0.04, 0.82, 0.001, 0.5];

const createPercentageList = (numbersArr) => {
  return numbersArr * 100 + "%";
};

const percentageArray = decimalArray.map(createPercentageList);
...
const createListOfPoessessions = (possessionsArr, name) => {
  name = "disco";
  return name + " " + possessionsArr;
};

const myPossessions = possessions.map(createListOfPoessessions);
console.log(myPossessions);
...
const convertStringToNumbersArray = (numberString) => {
  ...
  const arr = splitStr.map((num) => +num);
  return arr;
};
...
const createOddEvenArray = (numberString) => {
  ...
  let oddEvenArray = numberArray.map((number) =>
    number % 2 === 1 ? "odd" : "even"
  );
  return oddEvenArray;
};
...
// };
export const filterBooksBySearch = (booksArr, searchTerm) =>
  booksArr.filter((book) => book.includes(searchTerm));
