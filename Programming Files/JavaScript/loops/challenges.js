...
const getFirstAndLastItems = (itemsArr) => {
  let lengthArray = itemsArr.length;
  const firstLast = [itemsArr[0], itemsArr[lengthArray - 1]];
  return firstLast;
};
...
const totalScores = (scoreArr) => {
  return scoreArr.reduce((total, num) => total + num);
  return;
};
...
const moveFirstAndLastItems = (itemsArr) => {
  ...
  let newArray = [lastItem].concat(itemsArr);
  newArray.pop();
  return newArray;
  // ...itemsArr -
  // [...itemsArr] - copy of array
};

const moveFirstAndLastItems = (itemsArr) => {
  let newArray1 = [...itemsArr];
  newArray1.unshift(newArray1.pop());
  return newArray1;
};
...
const removeEvenNumbers = (numberArr) => {
  // copies array
  let arr = [...numberArr];
  const fin = arr.filter((num) => num % 2 != 0);
  return fin;
};
...
const generateAverage = (numberArr) => {
  let total = 0;
  let numberAverage = 0;
  numberArr.forEach((number) => (total += number));
  numberArr.length > 0
    ? (numberAverage = total / numberArr.length)
    : (numberAverage = 0);
  return Math.round(numberAverage);
};

// const generateAverage = (numberArr) => {
// let lengthArray = numberArr.length;
//   if (lengthArray === 0) {
//     return 0;
//   } else {
//     let sumArray = 0;
//     for (let i = 0; i < lengthArray; i++) {
//       sumArray += numberArr[i];
//     }
//     return Math.round(sumArray / lengthArray);
//   }
// };
...
const reverseOrder = (toReverseArr) => {
  ...
  for (
    let i = 0, j = toReverseArr.length - 1;
    i < toReverseArr.length;
    i++, j--
  ) {
    reverseArr[i] = toReverseArr[j];
    reverseArr[j] = toReverseArr[i];
  }
  return reverseArr;

  // let clonedArray = [...toReverseArr];
  // let reversedArray = clonedArray.reverse().map(number => number);
